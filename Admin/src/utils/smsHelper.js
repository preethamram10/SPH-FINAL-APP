...
const getBranchSmsDetails = (branch) => {
  ...
  if (b.includes('kphb') || b.includes('kukatpally')) {
    return {
      key: 'kphb',
  ...
  if (b.includes('chanda') || b.includes('chandn') || b.includes('lingampally')) {
    return {
      key: 'chanda',
  ...
  if (b.includes('dilshuk') || b.includes('dilsukh') || b.includes('dilsukhnagar')) {
    return {
      key: 'dilshuk',
...
const sendSmsRequest = async (mobile, message, templateId) => {
  try {
    const url = `${SMS_URL}?username=${encodeURIComponent(SMS_USERNAME)}&apikey=${encodeURIComponent(SMS_APIKEY)}&senderid=${encodeURIComponent(SMS_SENDERID)}&mobile=${encodeURIComponent(mobile)}&templateid=${encodeURIComponent(templateId)}&message=${encodeURIComponent(message)}`;
