...
const getFetchLogic = (isApp) => `
        ...
        const fetchTrueReached = async () => {
          try {
            ...
            pSnap.forEach(d => {
              const p = d.data();
              if (p.paymentStatus === 'paid' && normalizeBranch(p.branchId || p.branchName || p.branch) === targetNormalizedBranch) {
                if (parseDateMonth(p.paymentCollectedAt || p.appointmentDate || p.createdAt || p.date) === monthKey) {
                  ...
                  const consAmt = (() => {
                    if (hasMed && hasCons) return getExactPatientAmount(p);
            ...
            mSnap.forEach(d => {
              const data = d.data();
              if (data.paymentStatus === 'paid' && normalizeBranch(data.branchId || data.branchName || data.branch) === targetNormalizedBranch) {
...
updateWeb();
updateApp();
console.log("Updated both dashboards with exact logic");
